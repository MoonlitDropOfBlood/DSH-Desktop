...
function notifyTaskEvent(data) {
  ...
  let title;
  let body = "";
  // Every kind arrives only for a session's MAIN agent — the host-half plugin
  // filters subagent events on all three channels (done/error/approval), so
  // only main-session completion, failure, and pending confirmation notify.
  if (data.kind === "done") {
    title = "任务完成";
