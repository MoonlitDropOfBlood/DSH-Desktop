...
function prepareDesktopPlugin() {
  try {
    ...
    // copyDirRecursive (asar-safe primitives), NOT a flat readdirSync loop:
    // a subdirectory added to dsh-desktop-plugin/ would make copyFileSync
    // throw EISDIR in the packaged app (the source lives inside the asar),
    // the catch below would drop the WHOLE patch, and the window controls
    // would silently degrade to the fallback strip.
